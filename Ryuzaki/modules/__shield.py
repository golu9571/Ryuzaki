...
from Ryuzaki.conf import get_int_key, get_str_key

# from Ryuzaki.db.mongo_helpers.nsfw_guard import add_chat, get_all_nsfw_chats, is_chat_in_db, rm_chat
from Ryuzaki.pyrogramee.telethonbasics import is_admin
from Ryuzaki.events import register
from Ryuzaki import MONGO_DB_URI 
from pymongo import MongoClient
from Ryuzaki.modules.sql_extended.nsfw_watch_sql import (
    add_nsfwatch,
...
async def ws(event):
    ...
    if nude.is_nude("./nudes.jpg"):
        ...
        hh = sender.id
        final = f"**NSFW DETECTED**\n\n{st}](tg://user?id={hh}) your message contain NSFW content.. So, Ryuzakibot deleted the message\n\n **Nsfw Sender - User / Bot :** {st}](tg://user?id={hh})  \n\n`⚔️Automatic Detections Powered By RyuzakibotAI` \n**#GROUP_GUARDIAN** "
...
async def nsfw_watch(client, message):
    ...
    if not hot:
        ...
        await client.send_message(
            message.chat.id,
            f"**NSFW DETECTED**\n\n{hehe}'s message contain NSFW content.. So, Ryuzakibot deleted the message\n\n **Nsfw Sender - User / Bot :** `{Escobar}` \n**Chat Title:** `{ctitle}` \n\n`⚔️Automatic Detections Powered By RyuzakibotAI` \n**#GROUP_GUARDIAN** ",
        )
        message.continue_propagation()
"""
...
__help__ = """
<b> Group Guardian: </b>
✪ Ryuzakibot can protect your group from NSFW senders, Slag word users and also can force members to use English
